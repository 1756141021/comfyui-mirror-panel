...
### 跟 ComfyUI 原生 subgraph 的区别

简单说：subgraph **真的把节点搬走**，Mirror Panel **只是借着看一眼**。

| | 原生 subgraph | Mirror Panel |
|---|---|---|
| 节点物理位置 | 被搬进 wrapper 节点，原位置空了 | 一动不动，留在原工作流里 |
| 连线 | 跨越边界的线被截断、走 wrapper 的 IO slots 中转，**线在工作流里全部重新排** | 一条都不动 |
| 退出后想撤销 | 要 unpack subgraph，重新接线，节点 ID 全变 | 直接点 Canvas，原样还在 |
| 适合场景 | 你**真的想把这堆节点封装成一个可复用模块** | 你**只是想要一张操作面板** |

**最关键一条：subgraph 会让你工作流里原本整齐的连线乱飞**——线被切成"进 wrapper / 出 wrapper"两段，wrapper 节点位置又得腾出来，整片布局崩。Mirror Panel 不动你工作流里的任何一根线。
...
### How is this different from native ComfyUI subgraph?

In short: subgraph **physically moves your nodes**. Mirror Panel **just borrows a glance**.

| | Native subgraph | Mirror Panel |
|---|---|---|
| Where the nodes live | Moved into a wrapper node, original spot now empty | Stay exactly where they were |
| Wires | Wires crossing the boundary get cut, rerouted through the wrapper's IO slots — **your whole layout's wiring gets rearranged** | Untouched. Not a single wire moves |
| Undo / restore | Need to unpack subgraph, reconnect manually, node IDs all change | Just click Canvas — original layout is right there |
| When it makes sense | You **actually want to encapsulate these nodes as a reusable module** | You **just want a panel for the controls you keep touching** |

**The key one: subgraph turns your previously-tidy wiring into a tangle** — wires get split into "into wrapper / out of wrapper" segments, the wrapper itself takes up space, and the surrounding layout breaks. Mirror Panel touches zero wires in your workflow.
